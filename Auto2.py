...
def mon_heatmap(start,end):
    df=multi_year.loc[ (multi_year['NIBRSDescription'].isin(property_crimes))
    ...
    hm.save(f'''C:/Users/chris/Documents/Random_Nextdoor/My_Crime_Analysis - 05.04.23/Group/Auto_Theft_HeatMap_{start}.html''')
...
def map_plot(start,end):
    ...
    color_crime=dict(zip(property_crimes,all_colors))
    ...
    m.save(f'''C:/Users/chris/Documents/Random_Nextdoor/My_Crime_Analysis - 05.04.23/Group/Auto_Theft_{start}.html''')
...
def cluster_incidents(start,end,n_neur=12):
    X=multi_year.loc[ (multi_year['NIBRSDescription'].isin(property_crimes))
                          &(multi_year['RMSOccurrenceDate']>=f'''{start}/01/2023''')
                          &(multi_year['RMSOccurrenceDate']<f'''{end}/01/2023''')
    ...
    clu_m.save(f'''C:/Users/chris/Documents/Random_Nextdoor/My_Crime_Analysis - 05.04.23/Group/Auto_Theft_Cluster_{start}.html''')
...
def map_plot(start,end):
    ...
    m.save(f'''C:/Users/chris/Documents/Random_Nextdoor/My_Crime_Analysis - 05.04.23/Group/property_time.html''')
...
def premise( start
    ...
    agg_df.plot( kind='bar'
                ,x='Premise'
                ,figsize=(12,6)
                ,title=f'''property Incident Counts By Premise''')
    agg_df['percent']=(agg_df['OffenseCount']/(agg_df['OffenseCount'].sum()))*100.00
      
    print( 'Top 6 Premises For Auto Theft\n'
          ,agg_df[['Premise','OffenseCount','percent']]\
              .sort_values('OffenseCount',ascending=False)[['Premise','percent']].head(6))  
